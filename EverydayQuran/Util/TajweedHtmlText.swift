...
struct TajweedHtmlText {
     ...
     func tajweedText(for string: String, isDark: Bool = false) -> NSAttributedString {
        var attributedString = NSMutableAttributedString(string: "")
        let htmlString = "<html><head><style> \(TajweedHtmlText.tajweedCSS) </style></head><body> \(string) </body></html>"
    ...
    static func tajweedTextOnFinish(for string: String, isDark: Bool = false, onFinish: @escaping (NSMutableAttributedString) -> ()) {
        DispatchQueue.main.async {
            let string = "<html><head><meta charset=\"utf-8\"><style> \(TajweedHtmlText.tajweedCSS) </style></head><body> \(string) </body></html>"
               if let attributedString = try? NSMutableAttributedString(data:  Data(string.utf8), options: [.documentType: NSAttributedString.DocumentType.html], documentAttributes: nil) {
                attributedString.addAttributes([.font: UIFont.preferredFont(forTextStyle: .title3),
                                                .paragraphStyle: {
...
extension String {
    func convertHtml() -> NSAttributedString {
        let string = "<html><head><meta charset=\"utf-8\"><style> \(TajweedHtmlText.tajweedCSS) </style></head><body> \(self) </body></html>"
           if let attributedString = try? NSAttributedString(data:  Data(string.utf8), options: [.documentType: NSAttributedString.DocumentType.html], documentAttributes: nil) {
               return attributedString
           } else {
